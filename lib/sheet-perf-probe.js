/**
 * Foundry v14 — Character sheet (ActorSheetV2) performance probe (dnd4e).
...
function registerSettings() {
	game.settings.register(MODULE_ID, "sheetPerfTimingHooks", {
		name: "[dev] Sheet perf probe: timing hooks",
		hint: "Instrument every sheet render (performance marks/measures, an extra libWrapper frame around _renderHTML, one console line per open). Off by default — it costs measurable time on the path this module is meant to speed up. The on-demand APIs (sheetPerfBreakdownActor etc.) work either way. Requires a reload; sheetPerfEnableTimingHooks(true) attaches the hooks for the current session.",
		scope: "client",
		config: true,
		type: Boolean,
		default: false
...
const PROBE_ENTRY_PREFIXES = ["sheetPerf.", "ActorSheetV2._renderHTML"];
const PROBE_ENTRY_BUDGET = 600;
let probeEntriesSinceTrim = 0;

/** Marks/measures are never dropped by the browser; keep only a recent window of ours. */
function trimProbeEntries() {
	if (++probeEntriesSinceTrim < PROBE_ENTRY_BUDGET) return;
	probeEntriesSinceTrim = 0;
	const isOurs = (name) =>
		typeof name === "string" && PROBE_ENTRY_PREFIXES.some((prefix) => name.startsWith(prefix));
	for (const entry of performance.getEntriesByType("measure")) {
		if (isOurs(entry.name)) performance.clearMeasures(entry.name);
	}
	for (const entry of performance.getEntriesByType("mark")) {
		if (isOurs(entry.name)) performance.clearMarks(entry.name);
	}
}

let timingHooksRegistered = false;

function registerSheetTimingHooks() {
	if (timingHooksRegistered) return false;
	timingHooksRegistered = true;

	const onPreRender = (app) => {
	...
	Hooks.on("preRenderApplicationV2", onPreRender);
	Hooks.on("preRenderActorSheetV2", onPreRender);
...
/**
 * Attach the timing hooks for the current session without changing the setting — for a
 * measurement run (tools/foundry-sheet-e2e) or a console session.
 * @param {boolean} [on]
 */
function sheetPerfEnableTimingHooks(on = true) {
	if (!on) {
		console.log(`[${MODULE_ID}] Timing hooks cannot be detached at runtime; reload to drop them.`);
		return timingHooksRegistered;
	}
	const attached = registerSheetTimingHooks();
	console.log(
		`[${MODULE_ID}] Timing hooks ${attached ? "attached" : "already attached"}. The _renderHTML libWrapper probe needs the sheetPerfTimingHooks setting plus a reload.`
	);
	return true;
}

/**
 * Time ApplicationV2._renderHTML for actor sheets. Only registered when the timing-hooks setting
 * is on: it adds a libWrapper frame to every sheet render.
 *
 * @param {typeof import("/systems/dnd4e/module/applications/sheets/actor-sheet.mjs").default} [ActorSheet4e]
 */
function registerSheetPerfLibWrapperProbes(ActorSheet4e) {
	const ASV2 = foundry.applications.sheets?.ActorSheetV2;
	const sheetClass = ActorSheet4e ?? globalThis.dnd4e?.applications?.sheets?.ActorSheet4e;
	const targets = [
		...(sheetClass ? getActorSheet4eMethodTargets(sheetClass, "_renderHTML") : []),
	...
	return registerLibWrapperFirst(
		"sheet-perf-probe:_renderHTML",
		targets,
		async function (wrapped, ...args) {
			if (!ASV2 || !(this instanceof ASV2)) return wrapped.apply(this, args);
			const mark = `ActorSheetV2._renderHTML#${this.document?.id ?? "?"}#${performance.now()}`;
			...
			try {
				...
				try {
					...
					if (m && m.duration > 5) {
						log("core", "ActorSheetV2._renderHTML", {
 ...
 * Mirrors systems/dnd4e/module/documents/item.mjs#getChatData (0.9+ does not enrich inside getChatData;
 * ActorSheet4e._prepareContext enriches description / power cards separately).
 *
...
async function sheetPerfProfileGetChatDataPhases(itemRef, opts = {}) {
	...
	t = performance.now();
	htmlOptions.rollData = item.getRollData();
	const rollDataMs = performance.now() - t;
	...
	const enrichOpts = { ...htmlOptions, relativeTo: actor };
	t = performance.now();
	let enriched = "";
	try {
		enriched = await foundry.applications.ux.TextEditor.implementation.enrichHTML(description, enrichOpts);
	} catch (e) {
	...
	const out = {
		...
		phasesMs: {
			...
			getRollData: Number(rollDataMs.toFixed(2)),
			typeChatData: Number(typeChatDataMs.toFixed(2)),
			sheetLoopEnrichHTML: Number(enrichHTMLMs.toFixed(2)),
			fullGetChatData: Number(fullGetChatDataMs.toFixed(2))
		},
		hint: "dnd4e 0.9+ Item.getChatData is duplicate+labels+props only. enrichHTML cost is in ActorSheet4e._prepareContext (sheetLoopEnrichHTML here)."
	};
...
export function registerSheetPerfProbeReady() {
	...
	globalThis.sheetPerfEnableTimingHooks = sheetPerfEnableTimingHooks;

	// Off by default: instrumenting every render, and printing a baseline table on every world
	// load, both cost time on the exact path this module exists to speed up.
	const timingHooks = game.settings?.get(MODULE_ID, "sheetPerfTimingHooks") === true;
	if (timingHooks) {
	...
	console.log(
		`[${MODULE_ID}] Sheet perf probe ready (timing hooks ${timingHooks ? "on" : "off — sheetPerfEnableTimingHooks(true) or the [dev] setting"}). APIs: sheetPerfBreakdownActor(actor|id) → sheetPerfLastBreakdown; sheetPerfProfileGetChatDataPhases(item|id) → sheetPerfLastPhaseProfile; sheetPerfBenchmarkActor, sheetPerfPrewarmActor, sheetPerfBaseline, sheetPerfLog`
	);
}
